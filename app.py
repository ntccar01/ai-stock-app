...
import twstock
# import feedparser  <-- 兇手通常是他，我們先註解掉
import urllib.parse
...
# --- 1. 網頁設定 ---
st.set_page_config(page_title="AI 股市操盤手 Pro (穩定版)", layout="wide")
    ...
    if st.button("📡 快速掃描"):
        ...
        for i, code in enumerate(target_stocks):
            ...
            try:
                ...
                # 改用 requests session 來防止 Yahoo 阻擋
                session = requests.Session()
                session.headers.update({'User-Agent': 'Mozilla/5.0'})
                d = yf.download(code + scan_suffix, start=start_scan, progress=False, session=session)
